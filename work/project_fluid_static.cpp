...
void update_tile_inner(
    auto& tile, auto& prev_tile, Kernel<F>& kernel)
{
...
template <typename F>
void update_tile(auto& tile, auto& prev_tile, Kernel<F>& kernel)
{
    update_tile_simple(tile, prev_tile, kernel);
}

template <typename Tile, typename F>
void update_padding(Tile& tile, Kernel<F>& kernel)
{
    constexpr auto pad_x = Tile::pad_x();
    constexpr auto pad_y = Tile::pad_y();
    constexpr auto dim_x = Tile::dim_x();
    constexpr auto dim_y = Tile::dim_y();

    auto l = tile.template view<0, dim_x, 0, dim_y>();
    auto r = tile.template view<0, dim_x, dim_y - pad_y, dim_y>();
    auto t = tile.template view<0, pad_x, 0, dim_y>();
    auto b = tile.template view<dim_x - pad_x, dim_x, 0, dim_y>();
...
void save_to_bitmap(auto const& tile, const std::string& filename)
{
...
{
    const size_t dim_x = 512;    // Width of the simulation grid
    const size_t dim_y = 512;    // Height of the simulation grid
    ...
    using elem_t = Cell;
    using tile_t = Tile<elem_t, dim_x, dim_y, 1, 1>; // 1x1 padding
    // using iter_2d_t = tile_t::iterator_2d_t;
    ...
    auto k_external_forces = Kernel(1, 1,    // 1x1 kernel
        [&params](auto const& prev, auto& curr) -> void {
...
#undef INTERP_PROPERTY
    ...
    auto k_pressure_solver =
        Kernel(3, 3, [&params](auto const& prev, auto& curr) -> void {
            // using elem_t = typename iter_2d_t::value_type;
    ...
    auto k_make_some_fun =
        Kernel(1, 1, [&params](auto const& prev, auto& curr) -> void {
    ...
    for (size_t step = 0; step < params.sim_steps; ++step)
    {
        ...
        {
            // Update padding
            auto _ = timers.time_scope("Update padding");
            update_padding(tile, k_update_padding);
        }
        {
            // Apply external forces
            auto _ = timers.time_scope("External forces");
            update_tile(tile, prev_tile, k_external_forces);
        }

        {
            // Advection
            auto _ = timers.time_scope("Advection");
...

        {
            ...
            auto _ = timers.time_scope("Apply pressure forces");
            update_tile(tile, prev_tile, k_apply_pressure);
        }

        {
            // Make some fun oscillations
            auto _ = timers.time_scope("Fun");
            auto fun_view =
            ...
            auto prev_fun_view =
                prev_tile.inner<size_t(0), size_t(0.05 * tile.dim_x()),
                    size_t(0.45 * tile.dim_y()), size_t(0.55 * tile.dim_y())>();
            iter2_apply(fun_view.begin(), fun_view.end(), prev_fun_view.begin(),
                k_make_some_fun);
        }
