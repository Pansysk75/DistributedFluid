...
class Kernel{
public:
    Kernel(size_t dim_x, size_t dim_y, F f) : dim_x_(dim_x), dim_y_(dim_y), f_(f) {
    }
    template<typename T>
...
void update_tile_inner_seq(Tile<elem_t> &tile, Tile<elem_t> &prev_tile, Kernel<F> &kernel){
...
template <typename elem_t, typename F>
void update_tile_inner_par(Tile<elem_t> &tile, Tile<elem_t> &prev_tile, Kernel<F> &kernel){
    std::swap(prev_tile, tile);
    size_t n_threads = std::thread::hardware_concurrency();
    std::vector<std::future<void>> futures;
    ...
    for (size_t tid = 0; tid < n_threads; ++tid){
        ...
        futures.push_back(
            std::async(std::launch::async, [tid, &kernel](auto it, auto end, auto p_it){
...
void save_to_bitmap(Tile<float> const& tile, const std::string& filename) {
    ...
    auto idx = 0;
    std::for_each(tile.inner().begin(), tile.inner().end(), [&](const float& value) {
        // Assuming value is in range [0, 255] for grayscale
        auto px_val = static_cast<std::uint8_t>(std::clamp(value, 0.0f, 255.0f));
...
int main(){
    ...
    auto blur_kernel = Kernel(3,3, // 3x3 kernel
        ...
        // Blur kernel example
        auto sum = prev.get() + 
                   prev.get(-1, -1) + prev.get(-1, 0) + prev.get(-1, 1) +
                   prev.get(0, -1) + prev.get(0, 0) + prev.get(0, 1) +
                   prev.get(1, -1) + prev.get(1, 0) + prev.get(1, 1);
    ...
    std::generate(tile.begin(), tile.end(), [](){ return elem_t(rand() % 100); });
    std::generate(prev_tile.begin(), prev_tile.end(), [](){ return elem_t(rand() % 100); });
